...
public class TaskDemo {
    ...
    public void run(){
        ...
        Future<String> supplyTask = CompletableFuture.supplyAsync(supplier).thenApply(continuation).thenApply(continuation2);
        Future<?> task2 = CompletableFuture.supplyAsync(supplier2).thenApply(continuation2).thenAccept(acceptor);
    ...
    private Future<?> printNumber(int sum){
        return threadPool.submit(
        () -> {
            try {
                System.out.println("Task Starts for number: " + sum);
                Thread.sleep(1000);
